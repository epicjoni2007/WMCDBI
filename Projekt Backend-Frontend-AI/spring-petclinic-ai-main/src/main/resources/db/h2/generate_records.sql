...
-- choose ~80% of vets randomly and assign specialties (avoid window functions)
INSERT INTO vet_specialties (vet_id, specialty_id)
SELECT v.id, s.id
FROM (
    SELECT id FROM vets ORDER BY RAND() LIMIT 200
) v
CROSS JOIN (
    SELECT id FROM specialties
) s
WHERE NOT EXISTS (
  SELECT 1 FROM vet_specialties vs WHERE vs.vet_id = v.id AND vs.specialty_id = s.id
)
ORDER BY RAND()
LIMIT 300; -- approximately 2 specialties per vet
